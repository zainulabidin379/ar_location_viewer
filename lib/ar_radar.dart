...
class RadarPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final angleView = -angle;
    final angleView1 = angle;
    final center = Offset(radius, radius);
    final Paint paint = Paint()..color = background.withAlpha(100);
    final Path path = Path();
    final pointA = Offset(radius * (1 - sin(angleView)), radius * (1 - cos(angleView)));
    final pointB = Offset(radius * (1 - sin(angleView1)), radius * (1 - cos(angleView1)));
    ...
    final Paint paint2 = Paint()
      ..shader = RadialGradient(
        colors: [
          radarColor.withAlpha(168),
          radarColor.withAlpha(20),
        ],
  ...
  void drawMarker(Canvas canvas, List<ArAnnotation> annotations, double radius) {
    for (final annotation in annotations) {
      if (annotation.distanceFromUser > maxDistance) continue;
      final Paint paint = Paint()..color = annotation.color;
      final distanceInRadar = annotation.distanceFromUser / maxDistance * radius;
      final alpha = pi - (annotation.azimuth.toRadians - heading.toRadians);
